...
def extract_feat(g):
    ...
    version = "REF_IS32_BS134"
    last_epoch = [7, 13, 10, 9]
    ...
    epoch = 0
    device = 'cpu'
    batch_scores, A, feat = model.forward(g)
    return feat[0]
...
def get_graph_from_data():
    ...
    kf = StratifiedKFold(n_splits=5, shuffle=True) #KFold(n_splits=134*10, shuffle=True, random_state=42)
    ...
    categorical_len = len(df.drop(['fold', 'img', 'id'], axis=1)[categorical_var].drop([outcome,'age.splits', 'gender_concept_name'], axis=1).columns)
    unique_cat = []
    for cat_var in df.drop([ 'fold', 'img', 'id', ], axis=1)[categorical_var].drop([outcome, 'age.splits', 'gender_concept_name'], axis=1).columns:
    ...
    for i in tqdm(range(len(folds))):
        ...
        g.ndata['gender'] = torch.tensor(np.array(data['gender_concept_name']))
        #g.ndata['id'] = torch.tensor(np.array(data['id'].values.tolist()))
        g.ndata['label'] = torch.tensor(np.array(data[outcome].values.tolist()))
        g.ndata['ehr'] = torch.tensor(np.array(data[df.drop([outcome, 'fold', 'img', 'id', 'age.splits', 'gender_concept_name'], axis=1).columns].values.tolist()))
