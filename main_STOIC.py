...
def train_val_pipeline(MODEL_NAME, DATASET_NAME, params, net_params, dirs, tresh):
    ...
    return avg_val_acc, avg_train_acc
def main():
    """
        USER CONTROLS
    """
    ...
    parser.add_argument('--config', default = os.getcwd()+ "/configs/STOIC_pipeline.json", help="Please give a config.json file with training/model/data/param details")
    ...
    experiment_path = 'C:/Users/maxim/PycharmProjects/PrognosisGraph/experiments/network_analysis/'
    """list_of_rate = np.arange(0, 1.01, 0.1)
    list_of_exp = ['rate_global']#['custom']#['rate_1', 'rate_0', 'rate_group', 'rate_global']
    ...
    for exp in list_of_exp:
        ...
        plt.fill_between(x, mean_val - std_val, mean_val + std_val, alpha=0.5)#, edgecolor='#CC4F1B', facecolor='#FF9848')
    plt.legend(list_of_exp)
    plt.title('AUC score in function of positive rate')
    plt.xlabel('positive_rate of neigboors')
    plt.ylabel('AUC val score')
    plt.savefig(experiment_path + experiment_name + '.png')
    ""np.save(experiment_path + experiment_name, (val, train))
