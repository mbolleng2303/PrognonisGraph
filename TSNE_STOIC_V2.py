...
def train_val_pipeline(MODEL_NAME, DATASET_NAME, params, net_params, dirs, tresh):
    ...
    tsne = TSNE(n_components=n_components, init='random',
                random_state=0, perplexity=50, n_iter=5000)
    ...
    # 'COVID-19 Negatives' = labels == 0
    # 'COVID-19 Positives' = labels == 1

    # ax.scatter(Y[COVID, 0], Y[COVID, 1], c="r")
    # ax.scatter(Y[NON_COVID, 0], Y[NON_COVID, 1], c="b")
    import pandas as pd
    import seaborn as sns
    y = np.where(labels == 1, 'COVID-19 Positives', 'COVID-19 Negatives')
    ...
    # ax.legend(bbox_to_anchor=(1.05, 1), loc=8, borderaxespad=0.0)
