...
class Data2Graph(torch.utils.data.Dataset):
    ...
    def _prepare(self):
        t0 = time.time()
        path = os.getcwd()
        print("[I] Preparing graph without edge features...")
        ...
        sex = np.ones(400)
        labels = np.ones(400)
        feature = np.ones((400, 1024))
        index = [k for k, x in enumerate(df['Subset'].values.tolist()) if (x == 'holdout1')]
        data = df.iloc[index]
        age[0:200] = np.array(data['Age'].values.tolist())
        sex[0:200] = np.array(data['Sex'].values.tolist())
        feature[0:200, :] = np.array(list(map(eval, data['cls_Severe_VS_NSevere_feat_holdout1'].values)))
        labels[0:200] = np.array(data['Severity'].to_numpy(dtype=np.int8))
        index = [k for k, x in enumerate(df['Subset'].values.tolist()) if (x == 'holdout0')]
        data = df.iloc[index]
        age[200:400] = np.array(data['Age'].values.tolist())
        sex[200:400] = np.array(data['Sex'].values.tolist())
        feature[200:400, :] = np.array(list(map(eval, data['cls_Severe_VS_NSevere_feat_holdout0'].values)))
        labels[200:400] = np.array(data['Severity'].to_numpy(dtype=np.int8))
        g = dgl.DGLGraph()
        g.add_nodes(data.shape[0]*2)
        g.ndata['feat'] = torch.tensor(feature)
        g.ndata['Age'] = torch.tensor(age)
        g.ndata['Sex'] = torch.tensor(sex)
        g.ndata['Label'] = torch.tensor(labels)
...
class PreGraph(torch.utils.data.Dataset):
    ...
    def _prepare(self):
        ...
        for fold in folds:
            ...
            edge_feat = np.array(edge_feat)
            g.edata['feat'] = torch.tensor(edge_feat).long()
            edges_label = get_edge_labels(g)
            self.label_lists.append(edges_label)
            self.graph_lists.append(g)
...
class STOIC_Dataset(torch.utils.data.Dataset):
    def __init__(self, name):
        t0 = time.time()
        self.name = name
        save_dir = os.getcwd() + '/data/STOIC/save/'
    ...
    def _add_self_loops(self):
        ...
        for split_num in range(5):
            ...
            self.test[split_num] = DGLFormDataset(self.test[split_num].graph_lists,
                                                  self.test[split_num].label_lists)
