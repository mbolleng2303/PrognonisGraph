    ...
    try:
        if params['cross_val']:
            for split_number in range(4):
                ...
                with tqdm(range(params['epochs'])) as t:
                    for epoch in t:
                        ...
                        start = time.time()
                        epoch_train_loss, epoch_train_acc, optimizer, train_comb_loss, _, _ = train_epoch(model, optimizer, device,
                                                                                   train_loader, epoch)

                        epoch_val_loss, epoch_val_acc, val_comb_loss, _, _ = evaluate_network(model, device, val_loader, epoch)

                        epoch_test_loss, epoch_test_acc, test_comb_loss, _, _ = evaluate_network(model, device, test_loader, epoch)
            ...
            scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min',
                                                             ...
                                                             patience=params['lr_schedule_patience'],
                                                             verbose=True, min_lr=params['min_lr'])
            #scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=300, eta_min=params['min_lr'], verbose=True)

            epoch_tr_losses, epoch_gnn_losses = [], []
            epoch_tr_accs, epoch_gnn_accs = [], []
            ...
            # import train functions for all other GCNs
            from train.train_SBU import train_epoch_sparse as train_epoch, \
                evaluate_network_sparse as evaluate_network
            ...
            with tqdm(range(params['epochs'])) as t:
                for epoch in t:
                    ...
                    start = time.time()
                    epoch_train_loss, epoch_train_acc, optimizer, train_comb_loss, A_train, feat_train = train_epoch(model,
                                                                                                         optimizer,
                                                                                                         device,
                                                                                                         train_loader,
                                                                                                         epoch)

                    epoch_val_loss, epoch_val_acc, val_comb_loss, A_val, feat_val = evaluate_network(model, device, val_loader,
                                                                                           epoch)

                    epoch_test_loss, epoch_test_acc, test_comb_loss, A_test, _ = evaluate_network(model, device,
                                                                                               test_loader, epoch)
                    ...
                    current_epoch = {'val_acc': epoch_val_acc,
                                     ...
                                     'epoch': epoch,
                                     'test': epoch_test_acc,
                                     'sparsity_loss': test_comb_loss['sparsity'],
                                     'connectivity_loss': test_comb_loss['connectivity'],
                                     'smooth_loss': test_comb_loss['smooth'],
                                     'grl_loss': test_comb_loss['grl'],
                                     'gnn_loss': val_comb_loss['gnn'],
                                     'tr_loss': val_comb_loss['transformer'],
                                     'tr_acc': val_comb_loss['tr_acc'],
                                     'gnn_acc': val_comb_loss['gnn_acc'],
                                     'gt_loss': test_comb_loss['gt']}
                    if epoch == 0:
                        no_change = 0
                        save = True
                        best_epoch = current_epoch
                        ...
                        if not (current_epoch['val_acc'] >= best_epoch['val_acc']):
                            ...
                            no_change = 0
                            save = True
                    epoch_gnn_losses.append(current_epoch['gnn_loss'])
                    epoch_tr_losses.append(current_epoch['tr_loss'])
                    epoch_gnn_accs.append(current_epoch['gnn_acc'])
                    epoch_tr_accs.append(current_epoch['tr_acc'])
                    wandb.log(best_epoch)
                    img = A_val
                    min = float(img.min())
                    max = float(img.max())
                    ...
                    wandb.log({"A_val": images})
                    """i=0
                    #plt.figure()
                    fig, ax = plt.subplots(2)
                    for features in feat_train[:-1]:
                        perp = 15
                        ##tsne = TSNE(n_components)
                        tsne = TSNE(n_components=2, init='pca',
                                    random_state=0, perplexity=perp, n_iter=5000)
                        tsne_result = tsne.fit_transform(features)
                        y = np.where(feat_train[-1] == 1, 'label=1', 'label=0')
                        tsne_result_df = pd.DataFrame(
                            {'tsne_1': tsne_result[:, 0], 'tsne_2': tsne_result[:, 1], 'label': y})

                        sns.scatterplot(x='tsne_1', y='tsne_2', hue='label', ax=ax[i], data=tsne_result_df, s=120)
                    ...
                    scheduler.step(epoch_val_loss)
                    """if no_change > 3*params['lr_schedule_patience']+int(params['lr_schedule_patience']/2):
                        print('Best epoch since since {} epochs '.format(str(no_change)))
                        break"""
                    if optimizer.param_groups[0]['lr'] <= params['min_lr']:
                        print('LR min')
                    ...
                    if time.time() - t0_split > params[
                        ...
                        break
                plt.figure()
                plt.plot(epoch_gnn_losses)
                plt.plot(epoch_tr_losses)
                plt.legend(['gnn', 'transformer'])
                plt.ylabel('loss')
                plt.xlabel('epoch')
                wandb.log({"chart": wandb.Image(plt)})
                plt.figure()
                plt.plot(epoch_gnn_accs)
                plt.plot(epoch_tr_accs)
                plt.legend(['gnn', 'transformer'])
                plt.ylabel('acc')
                plt.xlabel('epoch')
                wandb.log({"chart": wandb.Image(plt)})
...
def main():
    ...
    parser.add_argument('--config', default=os.getcwd() + "/configs/SBU_HyperGraph.json",
    ...
    parser.add_argument('--lambda1', help="Please give a value for lambda")
    parser.add_argument('--lambda2', help="Please give a value for lambda")
    parser.add_argument('--lambda3', help="Please give a value for lambda")
    parser.add_argument('--lambda4', help="Please give a value for lambda")
    parser.add_argument('--lambda5', help="Please give a value for lambda")
    parser.add_argument('--lambda6', help="Please give a value for lambda")
    ...
    if args.pos_enc_dim is not None:
        net_params['pos_enc_dim'] = float(args.pos_enc_dim)
    ...
    if args.lambda5 is not None:
        net_params['lambda5'] = float(args.lambda5)
    if args.lambda6 is not None:
        net_params['lambda6'] = float(args.lambda6)
